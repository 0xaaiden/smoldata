...
function ContractStatusPage() {
    ...
    try {
        ...
        const progress = contractData.meta ? contractData.meta.progress.progress : 0;
        console.log("progress", progress)
        const startDate = new Date(contractData.startDate);
        const endDate = new Date(contractData.endDate);
        const eventsCount = contractData.meta ? contractData.meta.progress.eventsCount : 0;
        const events = contractData.events ? contractData.events : "";
        const fileSizeInMB = contractData.fileSize ? (contractData.fileSize / 1024 / 1024).toFixed(2) : 0;

        return (
            <div className="flex flex-col items-center">
                {/* add link top left Go back to events that takes you to / */}
                <Link to="/" className="relative place-self-start flex items-center top-0 left-0 ml-4 mb-20 text-blue-500 hover:text-blue-700">
                ...
                <div className="mt-8 w-full overflow-hidden">
                    <a href={`https://etherscan.io/address/${contractData.address}`} target="_blank" rel="noopener noreferrer">
                    ...
                    <div className="mt-4 w-full">
                        <div className="flex items-center">
                            ...
                            <div className="w-2/3 text-sm">{startDate.toLocaleDateString()}</div>
                        </div>
                        <div className="flex items-center">
                            <div className="w-1/3 text-sm text-gray-500">End Date:</div>
                            <div className="w-2/3 text-sm">{endDate.toLocaleDateString()}</div>
                        </div>
                        <div className="flex items-center">
                            <div className="w-1/3 text-sm text-gray-500">Events Indexed:</div>
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="w-2/3">
                                <div className="relative w-full h-2 bg-gray-200 rounded-full">
                                    <div
                                        className={`absolute top-0 left-0 h-2 rounded-full ${isComplete ? 'bg-green-500' : 'bg-yellow-500'
                                            }`}
                                ...
                                <div className="w-2/3">
                                    <a href={contractData.url} download>
                                        <button className=" font-bold py-2 px-4 ">
                                            Download - {fileSizeInMB ? `${fileSizeInMB} MB` : 'Calculating...'}
                                        </button>
                                    </a>
                                </div>
                            </div>
